...
RSpec.describe CommentsController, type: :controller do
  ...
  describe "POST #create" do
    ...
    let(:tour){FactoryBot.create :tour, category: category}
    let(:review){FactoryBot.create :review, tour: tour, user: user}
    let(:comment){FactoryBot.create :comment, user_id: user.id, review: review, commentable_type: "Review",commentable_id: review.id, content: "what's sub" }
    before do
      post :create, params: comment.attributes
    end
    it "update comment" do
      expect(comment.content).to eq("what's sub")  
    end
  end
end
